...
module.exports = function(db, store, payload, response){
	...
	xml += '<url><loc>https://####.####.####.####</loc><changefreq>weekly</changefreq></url>';
	db.collection('tax').find({},{_id:0}).toArray(function(err, docs){
        ...
        map(tax,function(val,key){
			xml += '<url><loc>https://####.####.####.####/' + key + '</loc><changefreq>weekly</changefreq></url>';
            map(val,function(subval,subkey){
				xml += '<url><loc>https://####.####.####.####/' + key + '/' + subkey + '</loc><changefreq>weekly</changefreq></url>';
                map(docs,function(value,index){
                    if(value.parent === subkey){
						xml += '<url><loc>https://####.####.####.####/' + key + '/' + subkey + '/' + value.n + '</loc><changefreq>weekly</changefreq></url>';
	...
	var crawlProducts = function(){
		db.collection('inventory').find({},{id: 1, department: 1, family: 1, line: 1, displayName: 1}).toArray(function(err, docs){
			map(docs, function(val, key){
				xml += '<url><loc>https://####.####.####.####/' + (val.department || 'd') + '/' + (val.family || 'f') + '/' + (val.line || 'l') + '/' + val.displayName.replace(/ /g,'-') + '-' + val.id + '</loc><changefreq>weekly</changefreq></url>';
			});
			xml += '</urlset>';
